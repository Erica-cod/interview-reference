...
>
> 现在第一轮仍固定让 Planner 产出结构化方案、Critic 给出评审，先建立可比较的基线。Critic 不只输出自然语言意见，还输出 `feasible / realistic / complete` 三项检查以及带严重级别的风险。Host 本身是确定性的 TypeScript 策略，不让模型自由决定流程：它按固定优先级读取轮次预算、本轮实际执行角色、高风险、三项检查、停滞轮数和首轮相似度，产生 `revise / challenge / verify / finalize / terminate` 五类动作。
>
> 例如还有高风险时，Host 返回 `revise`，下一轮只调度 Planner，并通过 `must_address` 把具体风险带过去；只要轮次预算允许，Planner 单独修订后不会直接结束，而是返回 `verify`，下一轮只让 Critic 验证。只有可行性、现实性、完整性全部通过、没有高风险且满足最小轮次，才进入 Reporter。达到最大轮次的判断优先级最高，即使最后一轮刚由 Planner 修订，也会 `terminate` 而不是再增加一轮，并让 Reporter 披露剩余风险。
>
> `next_agents` 现在已经是编排器的真实执行指令，不再只是展示字段；Host 的共识、风险、覆盖率和停滞趋势也会存进 `multi_agent_sessions`，断点恢复后继续使用。Embedding 相似度当前只有在“首轮结构已合格但过快一致”时参与路由，触发 Critic 压力测试；顽固 Agent 检测目前只做观测和展示，不要说成已经控制下一角色。
>
...
3. Host 按确定性策略决策：有高风险走 `revise`，预算允许时 Planner 修订后走 `verify`，首轮过快一致走 `challenge`；最大轮次优先 `terminate`。
...
```text
第一轮固定执行 Planner + Critic -> Host
达到轮次预算 -> terminate(Reporter披露风险)
仅Planner刚修订且预算允许 -> verify(Critic)
高风险或结构检查未通过 -> revise(Planner)
首轮结构合格但过快一致 -> challenge(Critic)
至少两轮且结构检查全部通过 -> finalize(Reporter)
```

### Host 到底何时选择哪个角色

Host 不是同时比较几个平级条件，而是按下面的短路顺序返回第一个命中的动作：

| 优先级 | 当前状态 | Host 动作 | 后续角色 |
| ---: | --- | --- | --- |
| 1 | `round >= maxRounds` | `terminate` | Reporter |
| 2 | 本轮执行了 Planner、没有执行 Critic | `verify` | Critic |
| 3 | 连续 2 轮无实质进展且仍有高风险 | `revise`，要求更换方案或关键假设 | Planner |
| 4 | 仍有任意高风险 | `revise`，逐项关闭风险 | Planner |
| 5 | 尚未达到最小轮次、结构检查通过且相似度 ≥ 0.9 | `challenge` | Critic |
| 6 | 至少 2 轮、结构检查通过且无高风险 | `finalize` | Reporter |
| 7 | `feasible / realistic / complete` 任一未通过 | `revise` | Planner |
| 8 | 其他兜底状态 | `verify` | Critic |

编排器每轮先从 `session.next_agents` 得到 `scheduledAgents`，只执行被选中的 Planner 或 Critic，然后固定再运行 Host。没有执行的角色会复用上一次输出，因此 Host 还必须读取 `executed_agents` 判断输出是否新鲜：Planner 单独修订时虽然上下文里仍有旧 Critic 输出，也不能用旧评审直接 `finalize`。

当前 `coverage` 只是三项布尔检查中通过项的比例，不是计划所有字段的细粒度覆盖率；三项全部为真时它就是 1。面试时不要把它包装成复杂评测指标。Reporter 也不是普通的“下一轮讨论角色”：`finalize / terminate` 会先结束讨论循环，再单独调用 Reporter。

### Host 专项验证

- 高相似但有高风险，不能结束。
- `next_agents=['critic']` 时，下一轮不能再执行 Planner。
- 轮次预算允许时，Planner 单独修订后必须由 Critic 验证。
- 达到最大轮次时，Reporter 能拿到终止原因和未解决风险。
- Embedding 失败时，文本相似度降级仍可完成决策。
- 当前 Host 与动态路由两组单测共 8 个用例通过。
- 尚需补“最大轮次恰好只有 Planner 修订”的测试；当前实现会优先 `terminate`，且 Reporter 尚无独立的 `unverified_revision` 字段。
...
### 高频追问：摘要会不会漏掉用户原消息里的时间

会。任何生成式摘要都可能漏掉日期、时刻、否定和数字，所以本项目的保证不是“摘要绝不丢时间”，而是“摘要绝不成为唯一事实源”：

1. 用户原消息始终保存在 MongoDB `messages`，摘要不会覆盖或删除原文。
2. `memory_items` 保留原文切片，`memory_summaries` 通过 `sourceMessageIds` 指回原消息。
3. 最近两轮强制使用完整原文。
4. 查询命中精确证据意图时，提高原文保真度、降低摘要权重，让具体时间从原文重新进入上下文。
5. 摘要失败或 Embedding 失败时仍可通过原文 BM25/关键词路径召回。

准确口述：

> 我不要求摘要模型百分之百保留时间，而是保留原文事实源。用户追问具体日期或时刻时，系统优先召回用户原话；摘要用于压缩和扩大语义召回，不能作为精确事实的唯一依据。
...
| 参数 | 普通摘要 | 精确日期/数字查询 | 原文 |
| --- | ---: | ---: | ---: |
| `fidelity` | 0.75 | 0.15 | 1 |
| fidelity 对 utility 的贡献 | 0.15 | 0.03 | 0.20 |
| 额外乘数 | 1 | 0.45 | 1 |

例如其他特征相近时，普通摘要效用约为 0.78，精确查询下可能降到 `0.66 × 0.45 ≈ 0.297`。随后再按 `utility / tokens^0.7` 装箱；`sourceMessageIds` 与最近原文或已选候选重叠时，还会继续施加冗余惩罚。

当前精确意图正则包含“具体、精确、原话、哪天、几点、日期、数字、错误码、版本号、路径、代码、deadline”等词，但尚未覆盖“什么时候、几号、周几、哪年、多久、时间、上午/下午”等常见问法。因此现阶段只能说已验证部分精确问法，不能说所有时间表达都有硬保证。

#### “重要性”当前怎么定义

原文切片的重要性是启发式规则，同一条消息的所有切片继承同一个分数：
...
命中“记住/以后/始终/必须/不要/偏好/目标/截止/
deadline/remember/always/prefer/must”  -> +0.25
长度在 80～1500 字符                     -> +0.05
最终上限                                  -> 1.00
```
...
```text
0.55
+ min(0.25, 目标/偏好/约束条数 × 0.05)
+ min(0.20, sourceMessageIds 数量 × 0.025)
```

这表示“更像长期约束、覆盖来源更多”，不等于业务影响大小，也不是模型训练出来的概率。当前时间格式本身不会自动加分，除非同时命中“截止/deadline”等关键词；这也是需要补齐的边界。

#### 怎么证明时间没有被忽略

当前离线回归里故意构造了摘要丢时间的反例：

```text
原文：申请材料截止日期是 2026 年 7 月 31 日 18:00，逾期不接受。
摘要：申请材料需要在本周内完成。
追问：我的申请材料具体哪天几点截止？
```

旧分开计分方案选中摘要和干扰项；当前统一计分选中原文。四组人工样本中，整体证据召回由 62.5% 提升到 100%，两个精确证据样本的原文召回由 0% 提升到 100%，重复 token 比例由 17.9% 降到 0%。本轮三个记忆测试文件共 15 个用例通过。

证据边界必须一起说：

- 时间类样本目前只有上面一组，其余精确样本是错误码。
- 所有计分实验候选使用同一个 `occurredAt`，没有验证新旧时间衰减排序。
- 测试证明的是“具体哪天几点”能取回原文，不证明摘要模型本身不漏时间，也不覆盖“什么时候”等同义问法。
- 默认相关性/时间/重要性权重 `0.7 / 0.1 / 0.2`、摘要 `0.15 / 0.45` 都是工程初值，不是生产数据证明的最优参数。

#### 参数应该怎么调

先建立带人工标签的时间查询集，至少覆盖绝对时间、相对时间、时间修改和冲突信息：
...
追问：什么时候 / 几号 / 周几 / 哪天下午 / 最后改成了几点
```

对 `summaryExactFidelity = 0 / 0.1 / 0.15 / 0.25 / 0.4` 和 `summaryExactMultiplier = 0.25 / 0.35 / 0.45 / 0.6 / 0.75` 做网格实验，在多档 token 预算下比较时间原文 Recall@K、最终时间 Exact Match、无关原文比例、重复率和 token 成本。选择满足时间原文召回目标时降权最小的组合，避免一遇到时间问题就塞入大量原文。

如果产品要求“时间绝不能漏”，不能只靠调参，应增加硬约束：识别到时间追问后，至少保留一条包含时间表达的用户原文；参数只负责其余候选排序。还应把时间意图词、常见日期/时刻格式加入检测，并补参数化回归。当前实现尚未完成这条硬约束。

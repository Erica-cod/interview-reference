...
先纠正问题里的概念：**本项目不是不用 RAG，而是在做面向会话记忆的 Hybrid RAG**。RAG 是“先检索，再把结果增强到生成上下文”的架构；BM25、向量检索、混合检索只是它的不同检索实现。这里没有直接套“文档切块 + 向量 TopK”的普通 RAG，原因是会话记忆和知识库问答的约束不同。

| 只用普通向量 RAG 的问题 | 混合方案如何处理 |
| --- | --- |
| 日期、错误码、版本号、路径、人名等精确字面证据，语义相似不等于精确命中 | BM25/全文检索负责精确词项，要求具体数字或原话时提高原文保真权重 |
| 用户换一种说法后，关键词可能完全不重合 | Embedding 负责语义召回，例如“静态类型方案”能找到“以后使用 TypeScript” |
| BM25 分数和余弦相似度量纲不同，直接加权很难稳定调参 | RRF 只融合两路排名，再叠加时间衰减和重要性 |
| 会话里的最新指令、长期偏好和普通闲聊价值不同 | 最近 2 轮强制保留；其他候选加入时间、重要性、来源保真度和信息密度 |
| 摘要与其来源原文可能一起占用预算 | 用 `sourceMessageIds` 计算重叠惩罚，并按 utility/token 成本装箱 |
| 向量索引或摘要任务失败会让纯向量链路断掉 | 降级到 BM25，再降级到 MongoDB 原始 `messages`；摘要从不作为唯一事实源 |

为什么不单独引入一个向量数据库：当前事实源已经在 MongoDB，Atlas 能提供全文和向量候选缩小；本地又能在有限候选内降级计算。当前规模下继续使用一套数据治理、权限和备份链路更简单。只有数据量、QPS、过滤能力或向量索引性能达到 MongoDB 的边界时，才值得拆出专用向量库，不能为了“用了 RAG”就多引入一个系统。

为什么不直接套通用 RAG 框架：通用文档 RAG 主要解决外部知识库问答，而这里还要处理最近两轮强制项、原文/摘要双表示、异步摘要状态、token 双账本、来源追溯和失败降级。这些属于会话记忆的业务规则，不能只靠一个 Retriever 的 TopK 参数表达。框架可以复用切片、Embedding 和 Retriever，但最终编排仍应留在业务层。

面试 60 秒回答：

> 严格来说我们没有放弃 RAG，这套方案本身就是会话记忆场景的 Hybrid RAG。只用向量 TopK 对语义改写很好，但对日期、错误码、版本和用户原话这类精确证据不稳定；只用 BM25 又找不到换一种说法的偏好。所以我让原文切片和摘要共同经过 BM25 与 Embedding，两路用 RRF 融合，再叠加时间、重要性、来源保真度和信息密度。最近两轮不参与竞争，其余内容按 utility/token 成本进入上下文，并用 sourceMessageIds 避免摘要和原文重复。Embedding、摘要或 Atlas 失败时还能退回全文检索和 Mongo 原文。因此这不是为了不用 RAG，而是把通用文档 RAG 改造成可追溯、可降级、受 token 预算约束的会话记忆 RAG。

如果追问“什么时候用普通 RAG”：外部文档是主要事实源、内容更新频率低、没有强会话状态和最近轮次优先级时，标准的文档切片 + 向量/BM25 TopK 已经足够；本项目复杂，是因为检索对象本身会随每轮对话持续写入和压缩。
